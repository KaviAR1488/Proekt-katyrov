...
backup_dir = "D:\Downloads\RASSIA"

class BackupSystem:
    def __init__(self):
        self.config_file = "backup_config.json"
        self.key_file = "backup_key.key"
    ...
    def restore_backup(self, backup_path, restore_dir="restored"):
        ...
        try:
            ...
            print("Убедитесь, что бэкап был создан с текущим ключом шифрования")
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Использование:")
        print("  python main.py full         - полный бэкап")
        print("  python main.py incremental  - инкрементный бэкап")
        print("  python main.py list         - список бэкапов")
        print("  python main.py restore <файл> - восстановить")
        print("  python main.py schedule     - запустить по расписанию")
    ...
    if command == "full":
        ...
        if len(sys.argv) < 3:
            ...
            print("Пример: python main.py restore backups/full_20241224_153045.zip")
            return
...
def run_demo(backup):
    ...
    backups = [f for f in os.listdir(backup.config["backup_dir"]) if f.startswith("full_")]
    if backups:
        backup_path = os.path.join(backup.config["backup_dir"], backups[0])
