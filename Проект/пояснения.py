...
class BackupSystem:
    def __init__(self):
        # Инициализация системы: загрузка конфигурации и создание ключа шифрования
    ...
    def load_config(self):
        # Этап 1: Загрузка конфигурации из файла или создание новой
    ...
    def save_config(self):
        # Этап 2: Сохранение обновленной конфигурации
        with open(self.config_file, 'w') as f:
            json.dump(self.config, f, indent=4)
    
    def get_file_hash(self, filepath):
        # Этап 3: Вычисление хэша файла для отслеживания изменений
        hasher = hashlib.md5()
    ...
    def get_changed_files(self, last_backup_time):
        # Этап 4: Поиск файлов, измененных после последнего бэкапа
        changed_files = []
    ...
    def full_backup(self):
        # Этап 5: Создание полной резервной копии всех файлов
        print("Создание полного бэкапа...")
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        backup_path = os.path.join(self.config["backup_dir"], f"full_{timestamp}.zip")
    ...
    def incremental_backup(self):
        # Этап 6: Создание инкрементной резервной копии (только измененные файлы)
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        backup_path = os.path.join(self.config["backup_dir"], f"inc_{timestamp}.zip")
    ...
    def create_archive(self, archive_path, file_list, backup_type):
        # Этап 7: Создание и шифрование ZIP-архива
    ...
    def restore_backup(self, backup_path, restore_dir="restored"):
        # Этап 8: Восстановление данных из резервной копии
    ...
    def list_backups(self):
        # Этап 9: Просмотр списка доступных резервных копий
    ...
    def set_schedule(self, interval_hours=24):
        # Этап 10: Настройка автоматического выполнения бэкапов по расписанию
        def job():
...
def main():
    # Основная функция: обработка командной строки и запуск соответствующих этапов
    backup = BackupSystem()
...
def run_demo(backup):
    # Этап 11: Демонстрационный режим - полный цикл работы системы
    ...
    print("Создано 2 тестовых файла")
    
    print("\n2. Создание полного бэкапа...")
    backup.full_backup()
    
    print("\n3. Изменение файла...")
    with open("data/file1.txt", "a") as f:
        f.write("\nДобавлена новая строка")
    
    print("\n4. Создание инкрементного бэкапа...")
    backup.incremental_backup()
    
    print("\n5. Список созданных бэкапов:")
    backup.list_backups()
    
    print("\n6. Восстановление из полного бэкапа...")
    backups = [f for f in os.listdir("backups") if f.startswith("full_")]
    if backups:
        backup.restore_backup(os.path.join("backups", backups[0]))
